...
class PythonConfiguration:
    ...
    def __validate_python(cls, version_major=3, version_minor=3):
        if sys.version is not None:
            ...
            if sys.version_info.major < version_major or (
                sys.version_info.major == version_major and sys.version_info.minor < version_minor):
                print("Python version too low, expected version {0:d}.{1:d} or higher.".format(version_major,
                                                                                               version_minor))
                return False
            return True
    ...
    def __install_package(cls, package_name):
        ...
        while not permission_granted:
            reply = str(
                input(
                    "Would you like to install Python package '{0:s}'? [Y/N]: ".format(package_name))).lower().strip()[
                    :1]
            if reply == 'n':
                return False
